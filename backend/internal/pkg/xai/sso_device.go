...
const (
	...
	// SSOConversionTimeout covers validate + device start retries (429 backoff) + approve + token poll.
	SSOConversionTimeout = 180 * time.Second
	...
	ssoDeviceStartMaxAttempts = 6
	ssoDeviceStartBaseBackoff = 1 * time.Second
	ssoDeviceStartMaxBackoff  = 30 * time.Second
)
...
func (f *ssoDeviceFlow) convert(ctx context.Context) (*TokenResponse, error) {
	...
	for attempt := 1; attempt <= ssoDeviceStartMaxAttempts; attempt++ {
		...
		if status == http.StatusTooManyRequests || status == http.StatusServiceUnavailable {
			if attempt >= ssoDeviceStartMaxAttempts {
				return nil, fmt.Errorf("start xAI device flow: %w", SSOHTTPError{Status: status})
			}
			backoff := ssoDeviceStartBackoff(attempt)
...
func ssoDeviceStartBackoff(attempt int) time.Duration {
	if attempt < 1 {
		attempt = 1
	}
	backoff := ssoDeviceStartBaseBackoff << (attempt - 1)
	if backoff > ssoDeviceStartMaxBackoff {
		return ssoDeviceStartMaxBackoff
	}
