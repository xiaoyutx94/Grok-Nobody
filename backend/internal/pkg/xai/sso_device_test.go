...
func (c *ssoDeviceFakeClient) Do(req *http.Request) (*http.Response, error) {
	...
	case SSODeviceURL:
		...
		if c.deviceCalls <= c.device429s {
			return ssoDeviceResponse(http.StatusTooManyRequests, nil, `{"error":"rate_limited"}`), nil
		}
...
func TestConvertSSOToBuildRetriesDeviceFlowRateLimit(t *testing.T) {
	t.Setenv(EnvClientID, "")
	client := &ssoDeviceFakeClient{t: t, device429s: 2}
	...
	require.NoError(t, err)
	require.Equal(t, "access-token", token.AccessToken)
	require.Equal(t, 3, client.deviceCalls)
	require.Equal(t, []time.Duration{time.Second, 2 * time.Second, time.Second}, sleeps)
}
