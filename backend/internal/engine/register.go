...
func (e *RegisterEngine) SaveConfig(ctx context.Context, cfg Config) (Config, error) {
        ...
        if cfg.CaptchaProvider == "" { cfg.CaptchaProvider = cur.CaptchaProvider }
        if cfg.EzSolverURL == "" { cfg.EzSolverURL = cur.EzSolverURL }
        if cfg.EzSolverTimeoutSec <= 0 { cfg.EzSolverTimeoutSec = cur.EzSolverTimeoutSec }
        // 0=auto 跟随注册并发是合法值（与 auralith_max_concurrency 一致）；
        // 只有负数才视为未设置、回退旧值
        if cfg.EzSolverMaxConcur < 0 { cfg.EzSolverMaxConcur = cur.EzSolverMaxConcur }
        if cfg.AuralithURL == "" { cfg.AuralithURL = cur.AuralithURL }
        if cfg.DefaultCount <= 0 { cfg.DefaultCount = cur.DefaultCount }
        if cfg.DefaultConcurrency <= 0 { cfg.DefaultConcurrency = cur.DefaultConcurrency }
        if cfg.DefaultEmailMode == "" { cfg.DefaultEmailMode = cur.DefaultEmailMode }
        if cfg.DefaultBrowser == "" { cfg.DefaultBrowser = cur.DefaultBrowser }
        if cfg.DefaultOSType == "" { cfg.DefaultOSType = cur.DefaultOSType }
        if cfg.CaptchaProxyMode == "" { cfg.CaptchaProxyMode = cur.CaptchaProxyMode }
        // paid captcha secrets: empty / *** keep previous
        if cfg.YesCaptchaKey == "" || cfg.YesCaptchaKey == "***" { cfg.YesCaptchaKey = cur.YesCaptchaKey }
        if cfg.NextCaptchaKey == "" || cfg.NextCaptchaKey == "***" { cfg.NextCaptchaKey = cur.NextCaptchaKey }
        if cfg.EzSolverAPIKey == "" || cfg.EzSolverAPIKey == "***" { cfg.EzSolverAPIKey = cur.EzSolverAPIKey }
        if cfg.AuralithAPIKey == "" || cfg.AuralithAPIKey == "***" { cfg.AuralithAPIKey = cur.AuralithAPIKey }
        // iCloud 取码平台：key 空/*** 保留旧值；地址空保留；超时/间隔 <=0 保留默认
        if cfg.IcloudAPIKey == "" || cfg.IcloudAPIKey == "***" { cfg.IcloudAPIKey = cur.IcloudAPIKey }
        if cfg.IcloudAPIBaseURL == "" { cfg.IcloudAPIBaseURL = cur.IcloudAPIBaseURL }
        if cfg.IcloudAPITimeoutSec <= 0 { cfg.IcloudAPITimeoutSec = cur.IcloudAPITimeoutSec }
        if cfg.IcloudAPIInterval <= 0 { cfg.IcloudAPIInterval = cur.IcloudAPIInterval }
        if cfg.AuralithTimeoutSec <= 0 { cfg.AuralithTimeoutSec = cur.AuralithTimeoutSec }
        // 0=auto 跟随注册并发是合法值；只有负数才视为未设置回退旧值
        if cfg.AuralithMaxConcur < 0 { cfg.AuralithMaxConcur = cur.AuralithMaxConcur }
        if cfg.EzCaptchaRetry <= 0 { cfg.EzCaptchaRetry = cur.EzCaptchaRetry }
        if cfg.DefaultCount <= 0 { cfg.DefaultCount = 1 }
        if cfg.DefaultConcurrency <= 0 { cfg.DefaultConcurrency = 1 }
        if cfg.EzSolverURL == "" { cfg.EzSolverURL = "http://127.0.0.1:8192" }
        if cfg.EzSolverTimeoutSec <= 0 { cfg.EzSolverTimeoutSec = 120 }
        if cfg.EzSolverMaxConcur < 0 { cfg.EzSolverMaxConcur = 0 }
        if cfg.EzCaptchaRetry <= 0 { cfg.EzCaptchaRetry = 5 }
        if cfg.DefaultEmailMode == "" { cfg.DefaultEmailMode = "mailtm" }
        if cfg.DefaultBrowser == "" { cfg.DefaultBrowser = "chrome" }
        if cfg.DefaultOSType == "" { cfg.DefaultOSType = runtimeGOOS() }
